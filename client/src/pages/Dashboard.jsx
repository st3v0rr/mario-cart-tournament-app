          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-sm)' }}>
            <div style={{ display: 'flex', gap: 'var(--spacing-md)', alignItems: 'center' }}>
              <span style={{ fontSize: '1.5rem', fontWeight: 700 }}>
                {formatTime(data.slot.start_time)}
                {clockSuffix ? ` ${clockSuffix}` : ''}
              </span>
              <span
                className={`badge badge-${data.slot.status === 'completed' ? 'success' : data.slot.status === 'booked' ? 'primary' : 'muted'}`}
              >
                {data.slot.status === 'completed'
            ...
            {data.slot.race_time && (
              <p>
                {t('dashboard.raceTime')} <strong>{data.slot.race_time}</strong>
              </p>
            )}
            {data.slot.status === 'booked' && (
            ...
            return (
              <div key={i} style={{ marginTop: i > 0 ? 'var(--spacing-sm)' : 0 }}>
                <p style={{ fontSize: '2rem', fontWeight: 700, margin: 0 }}>
                  ...
                  <span
                    style={{
                      color: b.round === 'final' ? 'var(--color-gold)' : 'var(--color-primary)',
                      marginLeft: 8,
                    }}
                  >
                    {b.round === 'semifinal' ? '🔥' : '🏆'}
